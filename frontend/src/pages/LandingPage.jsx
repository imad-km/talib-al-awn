...
const LandingPage = () => {
  ...
  useEffect(() => {
    ...
    const onScroll = () => {
      ...
      if (nb) {
        nb.style.boxShadow = window.scrollY > 10 ? '0 4px 24px rgba(16,32,45,.08)' : 'none';
      }
    };
    ...
    root.querySelectorAll('.pay-card-img').forEach(el => revealObserver.observe(el));
    ...
    if (carouselRoot) {
      ...
      if (body && slides.length) {
        let cfg = {};
        try { 
          cfg = JSON.parse(carouselRoot.getAttribute('data-carousel') || '{}'); 
        } catch {}
        ...
        if (cfg.isAutoPlay === true) {
          ...
          let interval = setInterval(() => go(1), speed);
          carouselRoot.addEventListener('mouseenter', () => { clearInterval(interval); interval = 0; });
          carouselRoot.addEventListener('mouseleave', () => { if (!interval) interval = setInterval(() => go(1), speed); });
    ...
    const mfn1 = root.querySelector('#mfn1');
    const mfn2 = root.querySelector('#mfn2');
    const mfn3 = root.querySelector('#mfn3');
    const mfp1 = root.querySelector('#mfp1');
    const mfp2 = root.querySelector('#mfp2');
    ...
    if (video) {
      const BP = 900;
      const TARGETS = [
        ['hero', 0.30, 0.50, 1.20], ['offer', 0.90, 0.20, 0.60],
        ['problem', 0.90, 0.50, 0.65], ['how', 0.90, 0.50, 0.65],
        ['app', 1.30, 0.50, 0.00], ['trust', 0.20, 0.20, 0.65],
        ['payment', 0.10, 0.50, 0.65], ['faq', 0.95, 0.50, 0.65],
        ['cta', 0.50, 0.42, 1.80]
      ];
      ...
      const tick = () => {
        ...
        cur.x = lerp(cur.x, tgt.nx, LERP); 
        cur.y = lerp(cur.y, tgt.ny, LERP); 
        cur.sz = lerp(cur.sz, tgt.sz, LERP);
        video.style.left = (cur.x * W) + 'px'; 
        video.style.top = (cur.y * H) + 'px'; 
        video.style.width = (cur.sz * H) + 'px';
      };
      tick();
    ...
    <div className="landing-root" dir={lang === 'ar' ? 'rtl' : 'ltr'} ref={rootRef}>
            ...
            {[1, 2, 3, 4].map((n) => (
              <div className="ps-row" key={n}>
                <div className="ps-half ps-half--problem">
                  ...
                  <div className="ps-half-body">
                    <span className="ps-dot ps-dot--red"></span>
                    <p>{t(`landing.ps.p${n}`)}</p>
                  </div>
                </div>
                <div className="ps-arrow-col">
                  <div className="ps-arrow-circle">{lang === 'ar' ? '←' : '→'}</div>
                </div>
                <div className="ps-half ps-half--solution">
                  <span className="ps-badge ps-badge--solution">{t('landing.ps.solution')}</span>
                  <div className="ps-half-body">
                    <span className="ps-dot ps-dot--green"></span>
                    <p>{t(`landing.ps.s${n}`)}</p>
                  </div>
                </div>
              </div>
            ))}
      ...
      <section id="how" className="mflow-section">
        <div className="mflow-inner">
          <div className="mflow-hdr">
            <div className="ps-bridge reveal">
              <div className="ps-bridge-line"></div>
              <span className="ps-bridge-label">{t('landing.how.bridge')}</span>
              <div className="ps-bridge-line"></div>
            </div>
          ...
          <div className="mflow-canvas-wrap">
            <div className="mflow-canvas" id="mflowCanvas">
              ...
              <div className="mf-node mf-node--1" id="mfn1">
                <div className="mf-sticky mf-sticky--green">
                  <div className="mf-sticky-label">{t('landing.how.step1Label')}</div>
                  <h3>{t('landing.how.step1Title')}</h3>
                  <p>{t('landing.how.step1Desc')}</p>
                </div>
                <div className="mf-mockup mf-mockup--jobs">
                  <div className="mf-mockup-bar">
                    <span className="mf-dot mf-dot--r"></span>
                    <span className="mf-dot mf-dot--y"></span>
                    <span className="mf-dot mf-dot--g"></span>
                    <span className="mf-mock-title">{t('landing.how.listingsTitle')}</span>
                  </div>
                  <div className="mf-mock-row mf-mock-row--a"></div>
                  <div className="mf-mock-row mf-mock-row--b"></div>
                  <div className="mf-mock-row mf-mock-row--a" style={{ width: '70%' }}></div>
                </div>
                <div className="mf-badge mf-badge--1"><span>{t('landing.how.student')}</span></div>
              </div>
              <div className="mf-node mf-node--2" id="mfn2">
                <div className="mf-sticky mf-sticky--yellow">
                  <div className="mf-sticky-label">{t('landing.how.step2Label')}</div>
                  <h3>{t('landing.how.step2Title')}</h3>
                  <p>{t('landing.how.step2Desc')}</p>
                </div>
                <div className="mf-mockup mf-mockup--chat">
                  <div className="mf-chat-bubble mf-chat-bubble--in">{t('landing.how.chatBubble1')}</div>
                  <div className="mf-chat-bubble mf-chat-bubble--out">{t('landing.how.chatBubble2')}</div>
                  <div className="mf-chat-bubble mf-chat-bubble--in">{t('landing.how.chatBubble3')}</div>
                </div>
                <div className="mf-badge mf-badge--2"><span>{t('landing.how.employer')}</span></div>
              </div>
              <div className="mf-node mf-node--3" id="mfn3">
                <div className="mf-sticky mf-sticky--orange">
                  <div className="mf-sticky-label">{t('landing.how.step3Label')}</div>
                  <h3>{t('landing.how.step3Title')}</h3>
                  <p>{t('landing.how.step3Desc')}</p>
                </div>
                <div className="mf-mockup mf-mockup--wallet">
                  <div className="mf-wallet-label">{t('landing.how.balanceLabel')}</div>
                  <div className="mf-wallet-amount">12,500 <span>DZD</span></div>
                  <div className="mf-wallet-bar"><div className="mf-wallet-fill" id="mfWalletFill"></div></div>
                  <div className="mf-wallet-chips">
                    <span className="mf-chip mf-chip--g">Edahabia</span>
                    <span className="mf-chip mf-chip--v">Chargily</span>
                  </div>
                </div>
              </div>
      ...
      <section id="app" className="app-bg">
        <div className="ai">
          <div>
            ...
            <div className="afs">
              <div className="af">
                <div className="afi"><img src="https://img.icons8.com/?size=100&id=7E1LwVS6HB4t&format=png&color=000000" alt="" style={{ width: 20, height: 20, filter: 'brightness(0) invert(1)' }} /></div>
                <div><div className="aft">{t('landing.app.f1Title')}</div><div className="afs2">{t('landing.app.f1Desc')}</div></div>
              </div>
              <div className="af">
                <div className="afi"><img src="https://img.icons8.com/?size=100&id=9xjQNFjDCyFj&format=png&color=000000" alt="" style={{ width: 20, height: 20, filter: 'brightness(0) invert(1)' }} /></div>
                <div><div className="aft">{t('landing.app.f2Title')}</div><div className="afs2">{t('landing.app.f2Desc')}</div></div>
              </div>
              <div className="af">
                <div className="afi"><img src="https://img.icons8.com/?size=100&id=qEK2pqenBa22&format=png&color=000000" alt="" style={{ width: 20, height: 20, filter: 'brightness(0) invert(1)' }} /></div>
                <div><div className="aft">{t('landing.app.f3Title')}</div><div className="afs2">{t('landing.app.f3Desc')}</div></div>
              </div>
            </div>
            <div className="sbs">
              <div className="sb-store">
                <img src="https://developer.apple.com/assets/elements/badges/download-on-the-app-store.svg" alt="App Store" height="40" style={{ display: 'block' }} />
                <div className="sb-coming">{t('landing.app.comingSoon')}</div>
              </div>
              <div className="sb-store">
                <img src="https://upload.wikimedia.org/wikipedia/commons/7/78/Google_Play_Store_badge_EN.svg" alt="Google Play" height="40" style={{ display: 'block' }} />
                <div className="sb-coming">{t('landing.app.comingSoon')}</div>
              </div>
            </div>
          </div>
          <div className="pc">
            <div className="pf">
              ...
              <div className="pbadge">
                ...
                <div className="btxt"><strong>{t('landing.app.newOffer')}</strong><span>{t('landing.app.cashier')} · 90 {t('landing.app.perDay')}</span></div>
              </div>
              <div className="pframe">
                <div className="pn"></div>
                <div className="pscr">
                  <img src="/assets/logo-07.svg" alt="Talib-Awn" style={{ background: 'transparent', boxShadow: 'none', width: 80, height: 80 }} />
                  <div className="pname">{t('landing.footer.footerTitle1')}-{t('landing.footer.footerTitle2')}</div>
                  <div className="psub">{t('landing.footer.footerSub')}</div>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="si">
          ...
          <div className="tg">
            <div className="tc reveal">
              <img src="https://img.icons8.com/?size=100&id=eoxMN35Z6JKg&format=png&color=000000" className="ti-img" alt="AI Verification" />
              <div className="tt">{t('landing.trust.f1Title')}</div>
              <p className="td">{t('landing.trust.f1Desc')}</p>
            </div>
            <div className="tc reveal rd1">
              <img src="https://img.icons8.com/?size=100&id=68930&format=png&color=000000" className="ti-img" alt="Secure Payments" />
              <div className="tt">{t('landing.trust.f2Title')}</div>
              <p className="td">{t('landing.trust.f2Desc')}</p>
            </div>
            <div className="tc reveal rd2">
              <img src="https://img.icons8.com/?size=100&id=wmiQxNp3EVI7&format=png&color=000000" className="ti-img" alt="Honest Reviews" />
              <div className="tt">{t('landing.trust.f3Title')}</div>
              <p className="td">{t('landing.trust.f3Desc')}</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <footer id="footer">
        <div className="fi">
          <div className="fg">
            ...
            <div className="fc">
              <h5>{t('landing.footer.platform')}</h5>
              <a href="#">{t('landing.footer.browse')}</a>
              <a href="#">{t('landing.footer.post')}</a>
              <a href="#">{t('landing.footer.pricing')}</a>
            </div>
            <div className="fc">
              <h5>{t('landing.footer.company')}</h5>
              <a href="#">{t('landing.footer.aboutUs')}</a>
              <a href="#how">{t('landing.nav.howItWorks')}</a>
            ...
            <div className="fc">
              ...
              <a href="#">{t('landing.footer.community')}</a>
            </div>
          </div>
        </div>
      </footer>
      ...
      <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', pointerEvents: 'none', zIndex: 2, isolation: 'isolate' }}>
        <video 
          ...
          style={{ position: 'absolute', pointerEvents: 'none', opacity: 0.9, mixBlendMode: 'screen', width: '20vw', transform: 'translate(-50%,-50%)' }} 
        />
      </div>
    </div>
  );
